...
`data/processed/features_matrix.csv`. **231 → 77 colunas**, sobre 15.587 imóveis.
...
| `bairro` | canonizado do endereço via `extrair_bairro` → **71 bairros oficiais** + `nao_informado` |
...
pegava a primeira palavra do endereço. Depois da canonização são **71 bairros
oficiais**. Ver [§9.7 de modelagem.md](modelagem.md).
...
| Arquivo | Conteúdo |
|---|---|
| `data/processed/features_matrix.csv` | a matriz consolidada, 15.587 × 77 |
