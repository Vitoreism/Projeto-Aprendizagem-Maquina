...
from imoveis_jp import config as projeto

# Diretórios e Arquivos
#
# Nada aqui é relativo ao arquivo nem ao diretório de onde o comando foi rodado:
# tudo sai de `imoveis_jp.config`, que é a regra nº 1 do README. Antes desta
# migração o scraper gravava dentro da própria pasta do módulo, e era por isso
# que existia uma cópia do JSON de anúncios dentro de `src/`.
#
# A separação segue o layout do projeto: DADO vai para data/raw (versionado,
# é o insumo do pipeline); ESTADO de execução vai para data/interim (fora do
# git — log, relatório, flag de pausa e sessão do navegador não são dado).
DIR_DADOS = str(projeto.RAW)
DIR_ESTADO = str(projeto.INTERIM)

ARQUIVO_SAIDA = str(projeto.ANUNCIOS_ZAP_JSON)
PAUSE_FLAG_FILE = os.path.join(DIR_ESTADO, "pause_zap.flag")
STORAGE_STATE_FILE = os.path.join(DIR_ESTADO, "session_state_zap.json")

# LOG_FILE e REPORT_FILE moram aqui, e não no logger.py, porque `scraper.py`
# sempre os importou DESTE módulo -- e como não existiam, o caminho de execução
# com um worker só (o default: total_workers=1, sem partição) quebrava com
# ImportError nas duas tentativas do try/except. A coleta nunca esbarrou nisso
# porque foi feita com vários workers, ramo que constrói os nomes na mão.
LOG_FILE = os.path.join(DIR_ESTADO, "scraping_zap.log")
REPORT_FILE = os.path.join(DIR_ESTADO, "execution_report_zap.json")
