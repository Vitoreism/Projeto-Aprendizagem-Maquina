...
try:
    from .config import (
        DIR_DADOS, DIR_ESTADO, USER_AGENT, EXTRA_HEADERS, TAMANHO_LOTE,
...
class ScraperEngine:
    ...
    def __init__(
        ...
        if partition_range:
            ...
            out_file = os.path.join(DIR_DADOS, f"imoveis_joao_pessoa_zap{suffix}.json")
            log_file = os.path.join(DIR_ESTADO, f"scraping{suffix}.log")
            report_file = os.path.join(DIR_ESTADO, f"execution_report{suffix}.json")
            pause_file = os.path.join(DIR_ESTADO, f"pause{suffix}.flag")
            self.storage_state_file = os.path.join(DIR_ESTADO, f"session_state{suffix}.json")
